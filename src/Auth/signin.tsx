...
const SignIn = () => {
  ...
  const submitHandler = (formData: z.infer<typeof signInformSchema>) => {
    ...
    <div className="bg-zinc-50 ">
      <MaxWidthWrapper className="min-h-[calc(100vh-56px-1px)] flex flex-col justify-center items-center">
        <div className="sm:w-[500px] w-full">
          <form className=" p-4 w-full flex flex-col gap-y-4 bg-white ring-1 ring-gray-200 rounded-lg" onSubmit={handleSubmit(submitHandler)}>
            ...
            <div className="flex flex-col gap-2">
              ...
              {errors.email && <p className="text-sm text-gray-500">{errors.email?.message}</p>}
            </div>

            <div className="flex flex-col gap-2">
              <Label>Password</Label>
              <Input type="password" placeholder="password" {...register('password')} />
              {errors.password && <p className="text-sm text-gray-500">{errors.password?.message}</p>}
            </div>
