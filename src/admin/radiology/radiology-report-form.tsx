...
interface RadiologyReportFormProps extends HTMLAttributes<HTMLDivElement> {
    editDetails: RadioTestReport
    isPending: boolean
...
const RadiologyReportForm = ({ Submit, isPending, editDetails, testNameId, ...props }: RadiologyReportFormProps) => {
    ...
    const fetchStaffs = async () => {
        try {
            const data = await getStaffs({ limit: 0, search: 'radiologist' }) // getting only staffs
            setStaffs(data)
        } catch ({ message }: any) {
            toast.error(message)
        }
    ...
    const fetchRadioTestNameParameters = async (testNameId: number) => {
        try {

            const data = await getRadioTestNameParameters(testNameId)
