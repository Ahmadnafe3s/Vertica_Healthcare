...
import { PathologyUnitType } from "../units/units"
import { getPathologytUnits } from "../api-handler"
...
export const CreatPathParameterSchema = z.object({
...
const CreatePathParameter = ({ editDetails, Submit, isPending, ...props }: Props) => {

    const { register, control, handleSubmit, formState: { errors } } = useForm<z.infer<typeof CreatPathParameterSchema>>({
        resolver: zodResolver(CreatPathParameterSchema),
        defaultValues: editDetails
    })


    const [units, setUnits] = useState<PathologyUnitType[]>([])
    ...
    return (
        <Dialog pageTitle="Add Pathology Parameter" className="sm:w-[500px] mx-auto" {...props}>
...
export default CreatePathParameter