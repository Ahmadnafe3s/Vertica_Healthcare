export const Companies = [
  { value: 'Sun Pharmaceutical', label: 'Sun Pharmaceutical' },
  { value: 'Cipla', label: 'Cipla' },
  { value: 'Dr. Reddy\'s Laboratories', label: 'Dr. Reddy\'s Laboratories' },
  { value: 'Lupin', label: 'Lupin' },
  { value: 'Aurobindo Pharma', label: 'Aurobindo Pharma' },
  { value: 'Glenmark Pharmaceuticals', label: 'Glenmark Pharmaceuticals' },
  { value: 'Torrent Pharmaceuticals', label: 'Torrent Pharmaceuticals' },
  { value: 'Cadila Healthcare', label: 'Cadila Healthcare' },
  { value: 'Biocon', label: 'Biocon' },
  { value: 'Mylan India', label: 'Mylan India' }
];


export const categories = [
  { value: 'Syrup', label: 'Syrup' },
  { value: 'Capsule', label: 'Capsule' },
  { value: 'Injection', label: 'Injection' },
  { value: 'Ointment', label: 'Ointment' },
  { value: 'Cream', label: 'Cream' },
  { value: 'Surgical', label: 'Surgical' },
  { value: 'Drops', label: 'Drops' },
  { value: 'Inhalers', label: 'Inhalers' },
  { value: 'Implants', label: 'Implants' },
  { value: 'Liquid', label: 'Liquid' },
  { value: 'Preparations', label: 'Preparations' },
  { value: 'Diaper', label: 'Diaper' },
  { value: 'Tablet', label: 'Tablet' }
];



export const group = [
  { label: 'Antibacterial', value: 'antibacterial' },
  { label: 'Antiviral', value: 'antiviral' },
  { label: 'Pain Reliever', value: 'pain_reliever' },
  { label: 'Anti-inflammatory', value: 'anti_inflammatory' },
  { label: 'Antifungal', value: 'antifungal' },
  { label: 'Hormonal', value: 'hormonal' },
  { label: 'Vitamins', value: 'vitamins' },
  { label: 'Antihistamine', value: 'antihistamine' },
  { label: 'Cardiovascular', value: 'cardiovascular' },
  { label: 'Diabetic', value: 'diabetic' },
  { label: 'Antiseptic', value: 'antiseptic' },
  { label: 'Cough Syrup', value: 'cough_syrup' },
  { label: 'Sleep Aid', value: 'sleep_aid' },
  { label: 'Vaccines', value: 'vaccines' },
  { label: 'Topical', value: 'topical' }
];


export const units = [
  { label: 'Milligram (mg)', value: 'mg' },
  { label: 'Milliliter (ml)', value: 'ml' },
  { label: 'Gram (g)', value: 'g' },
  { label: 'Liter (L)', value: 'l' },
  { label: 'Microgram (mcg)', value: 'mcg' },
  { label: 'Unit (U)', value: 'u' },
  { label: 'Pill', value: 'pill' },
  { label: 'Tablet', value: 'tablet' },
  { label: 'Capsule', value: 'capsule' },
  { label: 'Drop', value: 'drop' },
  { label: 'Syringe', value: 'syringe' },
  { label: 'Ounce (oz)', value: 'oz' },
  { label: 'Teaspoon (tsp)', value: 'tsp' },
  { label: 'Tablespoon (tbsp)', value: 'tbsp' }
];


export const roles = [
  { label: 'Admin', value: 'admin' },
  { label: 'Doctor', value: 'doctor' },
]

export const degisnations = [
  { label: 'Admin', value: 'admin' },
  { label: 'Doctor', value: 'doctor' },
]

export const departments = [
  { label: 'Admin', value: 'admin' },
  { label: 'OT', value: 'OT' },
  { label: 'Doctor Department', value: 'doctor department' },
  { label: 'OPD Department', value: 'OPD department' },
]


export const maritalStatus = [
  { label: "Single", value: "single" },
  { label: "Married", value: "married" },
  { label: "Divorced", value: "divorced" },
  { label: "Widowed", value: "widowed" },
  { label: "Separated", value: "separated" }
];


export const bloodGroups = [
  { label: "A+", value: "A+" },
  { label: "A-", value: "A-" },
  { label: "B+", value: "B+" },
  { label: "B-", value: "B-" },
  { label: "AB+", value: "AB+" },
  { label: "AB-", value: "AB-" },
  { label: "O+", value: "O+" },
  { label: "O-", value: "O-" }
];
