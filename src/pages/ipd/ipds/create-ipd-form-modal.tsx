    ...
    <Dialog pageTitle='Create IPD' {...props}>
      <form onSubmit={handleSubmit(Submit)}>
        <div className='flex  gap-2 px-2.5'>
          ...
          <div>
            <Controller name='patientId' control={control} render={({ field }) => {
              return <Select value={field.value ? String(field.value) : undefined} onValueChange={(value) => {
            ...
            <div className="w-full flex flex-col gap-y-2">
              <Controller control={control} name='bedGroupId' render={({ field }) => {
                ...
                return <>
                  ...
                  <Select value={field.value ? String(field.value) : undefined} onValueChange={(value) => { handleBedGroupChange(+value); field.onChange(value) }}>
                    <SelectTrigger >
                      <SelectValue placeholder="Select" />
                    </SelectTrigger>
                    ...
                    <SelectContent className='z-[200]'>
                      {bedGroups?.map((group, index) => (
                        <SelectItem key={index} value={String(group.id)}>{group.name}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </>
        ...
        <div className="flex mt-5 mb-2 gap-x-2 sm:justify-end px-2.5">
          ...
          <Button type='submit' className='flex-1 sm:flex-none' >Save {isPending && <Loader className='animate-spin' />}</Button>
        </div>
