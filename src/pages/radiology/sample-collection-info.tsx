...
const RadioSampleCollectionInfo = ({ excludeActions = false, isOpen, onClose, onDelete, ID }: Props) => {

    const { collectionForm, setCollectionForm, onCollectionSubmit, collectionDetails, getSampleCollectionById } = useRadiology()

    useEffect(() => {
        if (ID) getSampleCollectionById(ID)
    }, [isOpen, ID])
...
export default RadioSampleCollectionInfo